...
# Stremio Management Panel - VPS Installation Script for Node.js 16
# This script automates the installation of the Stremio Management Panel on a VPS server with Node.js 16
...
echo "${BOLD}${BLUE}"
echo "============================================================"
echo "    Stremio Management Panel - VPS Installer (Node.js 16)   "
echo "============================================================"
echo "${RESET}"
...
if ! command_exists node; then
  ...
  if [[ "$NODE_VERSION" =~ ^v16\. ]]; then
    print_success "Node.js 16.x already installed: $NODE_VERSION"
  else
    print_warning "You are using Node.js $NODE_VERSION. This script is optimized for Node.js 16.x."
    echo -n "${BOLD}${BLUE}Continue anyway? [y/N]: ${RESET}"
    read -r continue_anyway
    if [[ ! "$continue_anyway" =~ ^[Yy]$ ]]; then
      print_error "Installation aborted."
...
# Update frontend package.json to use compatible versions
print_status "Updating frontend package.json for Node.js 16 compatibility..."
